/**
 * ============================================================
 * SANTÉVIA TUNIS - JAVASCRIPT COMPLET
 * ============================================================
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    class CounterAnimator {
        ...
        init() { this.counters.forEach(c => this.observer.observe(c)); }
        animateCounter(element) {
            ...
            const updateCounter = (currentTime) => {
                ...
                if (hasPercent) element.textContent = currentValue + '%';
                else element.textContent = currentValue + '+';
                if (progress < 1) requestAnimationFrame(updateCounter);
                else element.textContent = hasPercent ? target + '%' : target + '+';
            };
    ...
    class TestimonialSlider {
        ...
        resetAutoSlide() { if (this.isAutoSliding) this.startAutoSlide(); }
        stopAutoSlide() {
    ...
    class ContactForm {
        ...
        async sendEmail(data) {
            ...
            if (this.PUBLIC_KEY && this.PUBLIC_KEY !== 'user_votre_cle') emailjs.init(this.PUBLIC_KEY);
            ...
            if (this.SERVICE_ID !== 'service_votre_id' && this.TEMPLATE_ID !== 'template_votre_id') {
        ...
        isValidPhone(phone) { return /^[\+\d\s\-\(\)]{8,20}$/.test(phone); }
    }
    ...
    if (newsletterForm) {
        newsletterForm.addEventListener('submit', function(e) {
            ...
            if (!email) { alert('Veuillez entrer votre adresse email.'); return; }
            if (!contactForm.isValidEmail(email)) { alert('Veuillez entrer une adresse email valide.'); return; }
    ...
    document.querySelectorAll('.whatsapp-float').forEach(link => {
        const phone = '21651555603';
        link.href = 'https://wa.me/' + phone + '?text=Bonjour%2C%20je%20souhaite%20obtenir%20des%20informations%20concernant%20Sant%C3%A9via%20Tunis.';
    });
    ...
    class DevisForm {
        ...
        async handleSubmit(e) {
            ...
            try {
                ...
                document.querySelectorAll('.merci-section').forEach(el => el.classList.add('active'));
                document.querySelectorAll('.contact, .devis, .faq, .guide, .recommandations, .stats-section, .about, .testimonials-preview, .tunisia-destination, .advantages, .services-preview, .hero, .cta-section').forEach(el => el.style.display = 'none');
                window.scrollTo({ top: 0, behavior: 'smooth' });
            } catch (error) {
    ...
    // --- BOUTON RETOUR ACCUEIL (Merci) ---
    const backToHomeBtn = document.getElementById('backToHome');
    if (backToHomeBtn) {
        backToHomeBtn.addEventListener('click', function() {
            document.querySelectorAll('.merci-section').forEach(el => el.classList.remove('active'));
            document.querySelectorAll('.contact, .devis, .faq, .guide, .recommandations, .stats-section, .about, .testimonials-preview, .tunisia-destination, .advantages, .services-preview, .hero, .cta-section').forEach(el => el.style.display = '');
            window.scrollTo({ top: 0, behavior: 'smooth' });
        });
    }

    // --- BOUTON ACHAT GUIDE ---
    const showGuidePaymentBtn = document.getElementById('showGuidePayment');
    if (showGuidePaymentBtn) {
        showGuidePaymentBtn.addEventListener('click', function() {
            alert('🔗 Redirection vers la page de paiement Stripe...');
